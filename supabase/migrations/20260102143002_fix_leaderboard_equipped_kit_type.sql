/*
  ...
  1. Problem
    - Function declares `equipped_kit_colors JSONB` but returns `TEXT[]`
    - PostgreSQL strict type checking causes: "Returned type text[] does not match expected type jsonb"

  2. Solution
    - Change return type from JSONB to TEXT[]
    - This matches the actual data type of house_kits.color_scheme

  3. Impact
    - Fixes leaderboard loading errors
    - All stat types (most_wins, best_accuracy, winning_streak) work correctly
*/

-- Drop and recreate function with correct type
DROP FUNCTION IF EXISTS get_house_leaderboard_stats(UUID, TEXT, INT);
...
BEGIN
  ...
  ELSIF stat_type = 'best_accuracy' THEN
    ...
    WHERE gs.house_id = house_id_param
      ...
      AND gs.is_solo_game = false
    GROUP BY ss.user_id, p.username, ups.profile_photo_url, hk.color_scheme
    HAVING COUNT(DISTINCT ss.session_id) >= 3
